...
- **UPDATE 2026-06-06 — Path 2 IMPLEMENTED (cutover done, via grounding not replacement).** Reprioritized at Jim's direction ("that's the whole point" — functional real-corpus RAG > stable interim eval). The PDQ corpus is now LIVE in the policy mapper as a **clinical-evidence grounding layer**: criteria still come from `nccn_nsclc_v5` (the rules, `criteria[]` schema unchanged), and the mapper additionally retrieves PDQ passages (`pdq_nsclc_v1`, 1,737 chunks, semantic) injected as `clinical_reference` to ground criterion reasoning. PDQ = evidence, not criteria (prompt forbids adding to `criteria`/`evidence_ref`). This is the *correct* cutover shape: PDQ is clinical reference-knowledge, not authorization criteria, so it plays the grounding role — and the criteria-status schema + eval dims + GT-audit labels all stay valid (NOT destabilized, contrary to the original deferral rationale). Forensic provenance: each case's `tool_calls_made` records the PDQ passages that grounded it. ADR-019 addendum. Files: `rag/chroma_retriever.py`, `agents/policy_mapper/agent.py`, `prompts/policy_mapper.md` (+hash), `tests/test_pdq_ingestion.py`. Real-corpus RAG is now genuinely functional and live.
...
- **Outcome honesty — UPDATED 2026-06-06 after Path 2: RAG is now functional and its outcome effect is MEASURABLE (not yet measured).** Before Path 2 this said RAG was infrastructure with no outcome effect (true then: it retrieved the same 12 criteria a lookup would). After Path 2, real-corpus retrieval genuinely informs the mapper's reasoning, so the next eval vs `eval_report_20260606_005031` IS the controlled before/after. We still do NOT claim improvement — it must be measured, and may help (better-grounded judgments) or hurt (noisier prose context). Original position retained below for the record: Two reasons, both honest: (1) the PDQ real-RAG is not the live path, so its outcome impact is zero by construction; (2) the live Phase 3b Chroma RAG retrieves the *same 12 criteria* the fixtures did — changing *how* content is fetched, not *what*, so no clinical-outcome delta is expected at this corpus size. This is consistent with the 2026-05-27 RAG-cut finding (*"none of the strategy/PRD outcome targets depend on RAG"*). **RAG's outcome value is a SCALE hypothesis** — at POC corpus size (12 criteria / pre-selected) retrieval quality cannot matter; it could matter over a large un-curated corpus (the 1,737-chunk PDQ cutover would test it, and might *hurt* short-term before the not_applicable/GT fixes). A clean A/B (retrieval ON vs OFF, same corpus) was considered and declined: the result is predictable (~neutral) and the project already documented it; spending a 75-min run to confirm a known-neutral result is not worth it. The completion gain (0.21→0.47) was the **index-dedup bug fix**, NOT RAG — do not attribute it to RAG.
