...
1. **Parse:** `lxml` over the cancer.gov HTML `<article>` node; segment by `h1–h3` headings into sections; drop nav/scripts/figures/tables and sub-200-char fragments. Output: `rag/pdq_corpus/nsclc_hp.json` (committed — public-domain text, with provenance) so the build is reproducible without re-fetching.
